...
/* After clicking on the invite button, the invite button will response. 
 Note: Player 1 clicked on is now invited */
socket.on('invite_response', (payload) => {
    ...
    /* change your button to show invite to invited */
    let newNode = makeInvitedButton(payload.socket_id);
    $('.socket_' + payload.socket_id + ' button').replaceWith(newNode);
})

/* Player 2 is getting the invited button
Note: Play button down */
socket.on('invited', (payload) => {
    ...
    /* replace existing button to newNode */
    $('.socket_' + payload.socket_id + ' button').replaceWith(newNode);
})

/* uninvite and get uninvited message back */
...
Note: Wait for server to reponse (navigateto join room response(scroll up))  */
$(() => {
    let request = {};
    request.room = chatRoom;
    request.username = username;

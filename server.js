...
/* if we aren't on Heruku, then we need to adjust our port and directory */
...
io.on('connection', (socket) => {
    ...
    */
    ...
    socket.on('invite', (payload) => {
        ...
        /* note: allSockets to fetchSockets*/
        io.in(room).allSockets().then((sockets) => {
            /* Invitee isn't in the room */
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.has(requested_user)) {
                /* note: || !sockets.has(requested_user) */
                response = {
                    result: 'fail',
    ...
    socket.on('uninvite', (payload) => {
        ...
        io.in(room).allSockets().then((sockets) => {
            ...
            /* UnInvitee is in the room */
                ...
                /* send message back to the player who was uninvited */
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null) || (room === "")) {
            ...
            socket.emit('game_start_reponse', response);
            serverLog('game_start command failed', JSON.stringify(response));
            return;
        }
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
    ...
    */
    ...
    /* payload is the message from the client that someone has clicked on the board and then server is checking the command*/
    socket.on('play_token', (payload) => {
        ...
        /* Make sure the current attempt is by the correct color- the right person playing */
        ...
        if (
            ...
            let response = {
                resule: 'fail',
                message: 'play_token played the right color, but by the wrong player. It\'s not their turn'
            }
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
...
function create_new_game() {
    ...
    /* game rules- restriction on the white and black moves to match the current rule */
...
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* Check to makte sure that there is space for a matching color to capture tokens */
